...
public class TicketTransitionMap {
    ...
    public List<Action> getActions(TicketViewDto ticket, User user) {
        Map<State, Map<UserRole, List<Action>>> actionStateMap = Map.of(
                State.DRAFT, Map.of(
                        UserRole.EMPLOYEE, new ArrayList<>() {{add(Action.SUBMIT); add(Action.CANCEL);}},
                        UserRole.MANAGER, new ArrayList<>() {{add(Action.SUBMIT); add(Action.CANCEL);}}
                        )
                ,
                State.NEW, Map.of(
                        UserRole.MANAGER, new ArrayList<>() {{add(Action.APPROVE); add(Action.DECLINE); add(Action.CANCEL);}}
                )
                ,
                State.APPROVED, Map.of(
                        UserRole.ENGINEER, new ArrayList<>() {{add(Action.ASSIGN_TO_ME); add(Action.CANCEL);}}
                )
                ,
                State.DECLINED, Map.of(
                        UserRole.EMPLOYEE, new ArrayList<>() {{add(Action.SUBMIT); add(Action.CANCEL);}},
                ...
                ,
                State.IN_PROGRESS, Map.of(
                        UserRole.ENGINEER, new ArrayList<>() {{add(Action.DONE);}}
                ),
                State.DONE, Map.of(
                        UserRole.ENGINEER, new ArrayList<>() {{add(Action.DONE);}}
                )
